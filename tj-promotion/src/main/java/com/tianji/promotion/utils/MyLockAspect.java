...
public class MyLockAspect implements Ordered {
    ...
    public Object tryLock(ProceedingJoinPoint pjp, MyLock myLock) throws Throwable {
        //1.获取锁对象
        RLock lock = redissonClient.getLock(myLock.name()); //获取锁对象 默认是可重入锁

        String lockName = getLockName(myLock.name(), pjp);

        log.debug("获取到的分布式锁的key: {}", lockName);
        //根据工厂模式 获取用户指定类型的 锁对象
        myLockFactory.getLock(myLock.lockType(), lockName);


        //2.尝试获取锁
        boolean isLock = myLock.lockStrategy().tryLock(lock, myLock); //使用策略工厂模式 让用户使用需要的策略模式
        //3.判断是否成功
        if (!isLock) {
            return null;
        }
